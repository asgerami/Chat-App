      ...
      <div className="grid grid-cols-2">
        ...
        <div className="flex min-w-32 md:min-w-64 flex-col gap-5 text-white items-center justify-center">
          ...
          <div className="w-full flex gap-5">
            {
              colors.map((color, index) => <div className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 ${selectedColor === index ? "outline outline-white/100 outline-1" : ""}`} key={index} onClick={(e) => {setSelectedColor(index)}}></div>)
            }
          </div>
        </div>
      </div>
